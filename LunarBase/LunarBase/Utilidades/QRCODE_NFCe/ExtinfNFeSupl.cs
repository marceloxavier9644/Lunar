...
using static LunarBase.Utilidades.QRCODE_NFCe.Conversao;
...
namespace LunarBase.Utilidades.QRCODE_NFCe
{
    ...
    public static class ExtinfNFeSupl
    {
        ...
        /// </summary>
        public static string ObterUrlConsulta(this infNFeSupl infNFeSupl, TNFe nfe, VersaoQrCode versaoQrCode)
        {
            //TipoAmbiente tpAm = new TipoAmbiente();
            //if (Sessao.parametroSistema.AmbienteProducao == true)
            //    tpAm = TipoAmbiente.Producao;
            //else
            //    tpAm = TipoAmbiente.Homologacao;


            var versaoServico = Conversao.StringParaVersaoServico(nfe.infNFe.versao);
            return ObterUrl(infNFeSupl, (TipoAmbiente)nfe.infNFe.ide.tpAmb, (Estado)nfe.infNFe.ide.cUF, TipoUrlConsultaPublica.UrlConsulta, versaoServico, versaoQrCode);
        }

        /// <summary>
        ///     Obtém a URL para montagem do QR-Code
        /// </summary>
        public static string ObterUrlQrCode(this infNFeSupl infNFeSupl, TNFe nfe, VersaoQrCode versaoQrCode, string cIdToken, string csc)
        {
        ...
        /// </summary>
        //private static string ObterUrlQrCode1(infNFeSupl infNFeSupl, TNFe nfe, string cIdToken, string csc, VersaoServico versaoServico)
        //{
        ...
        //    var digVal = Conversao.ObterHexDeString(nfe.Signature.SignedInfo.Reference.DigestValue.ToString());

        //    //Na hipótese do consumidor não se identificar, não existirá o parâmetro cDest no QR Code;
        //    var cDest = "";
        //    if (nfe.infNFe.dest != null)
        //        cDest = "&cDest=" + nfe.infNFe.dest.Item + nfe.infNFe.dest.CNPJ + nfe.infNFe.dest.idEstrangeiro;
        ...
        /// </summary>
        private static string ObterUrlQrCode2(infNFeSupl infNFeSupl, TNFe nfe, string cIdToken, string csc, VersaoServico versaoServico)
        {
            #region 1ª parte

            var url = ObterUrlQrCode2ComParametro(infNFeSupl, (TipoAmbiente)nfe.infNFe.ide.tpAmb, (Estado)nfe.infNFe.ide.cUF, versaoServico);
            ...
            if ((TipoEmissao)nfe.infNFe.ide.tpEmis == TipoEmissao.teOffLine)
            {
                var diaEmi = DateTime.Parse(nfe.infNFe.ide.dhEmi).Day.ToString("D2");
                var valorNfce = decimal.Parse(nfe.infNFe.total.ICMSTot.vNF).ToString("0.00").Replace(',', '.');
                var digVal = Conversao.ObterHexDeString(nfe.Signature.SignedInfo.Reference.DigestValue.ToString());
